...
from data_process import split_regression_data, move_file
from graphsformerCPI import graphsformerCPI
from my_dataset import Regression_DataSet
from multi_train_utils.distributed_utils import init_distributed_mode, dist, cleanup
from multi_train_utils.train_eval_utils import regression_train_one_epoch_skt_version, regression_evaluate_skt_version

'''
KIBA数据集中最大的化合物特征长374
## 多GPU启动指令
- 如果要使用```regression_multi_gpu.py```脚本，使用以下指令启动
...
- CUDA_VISIBLE_DEVICES=1 python -m torch.distributed.launch --nproc_per_node=1 --use_env regression_multi_gpu.py
CUDA_VISIBLE_DEVICES=1 python -m torch.distributed.launch --nproc_per_node=1 --master_port=25641 --use_env regression_multi_gpu.py
'''
...
def main(args):
    ...
    train_sample_ids, val_sample_ids, cpd_max_len, prt_max_len = split_regression_data(
        args.data_path, fold=args.fold, rank=0, cpd_threshold=args.cpd_threshold, prt_threshold=args.prt_threshold)

    # 实例化训练数据集
    train_data_set = Regression_DataSet(root=args.data_path, sample_ids=train_sample_ids, cpd_cut_len=cpd_max_len,
                                        prt_cut_len=prt_max_len)

    # 实例化验证数据集
    val_data_set = Regression_DataSet(root=args.data_path, sample_ids=val_sample_ids, cpd_cut_len=cpd_max_len,
                                      prt_cut_len=prt_max_len)
    ...
    if not os.path.exists(pred_true_value_dir):
        os.makedirs(pred_true_value_dir)  # 创建路径,存放pred_true_矩阵
    ...
    for epoch in range(args.epochs):
        ...
        train_scores, train_mean_loss = regression_train_one_epoch_skt_version(model=model,
                                                                               optimizer=optimizer,
        ...
        if rank == 0:
            print(
                "Train: [Epoch {}] [mean_loss:{:3f}]\t[rmse:{:3f}]\t[mse:{:3f}]\t[pearson:{:3f}]\t[spearman:{:3f}]\t[CI:{:3f}]".format(
                    epoch, train_mean_loss, train_scores['rmse'], train_scores['mse'], train_scores['pearson'],
                    train_scores['spearman'], train_scores['ci']))
        ...
        test_scores, test_mean_loss = regression_evaluate_skt_version(model=model, data_loader=val_loader,
                                                                      criterion=loss_function, device=device,
                                                                      epoch=epoch, logs=logs_test,a_v_dir=a_v_dir)
        if rank == 0:
            print(
                "Test: [Epoch {}] [mean_loss:{:3f}]\t[rmse:{:3f}]\t[mse:{:3f}]\t[pearson:{:3f}]\t[spearman:{:3f}]\t[CI:{:3f}]".format(
                    epoch, test_mean_loss, test_scores['rmse'], test_scores['mse'], test_scores['pearson'],
                    test_scores['spearman'], test_scores['ci']))

            tags = ["loss", "rmse", "mse", "pearson", "spearman", "CI", "learning_rate"]
            tb_writer.add_scalar(tags[0], test_mean_loss, epoch)
            tb_writer.add_scalar(tags[1], test_scores['rmse'], epoch)
            tb_writer.add_scalar(tags[2], test_scores['mse'], epoch)
            tb_writer.add_scalar(tags[3], test_scores['pearson'], epoch)
            tb_writer.add_scalar(tags[4], test_scores['spearman'], epoch)
            tb_writer.add_scalar(tags[5], test_scores['ci'], epoch)
            tb_writer.add_scalar(tags[6], optimizer.param_groups[0]["lr"], epoch)

            if best_metric > test_scores['mse']:
                best_metric = test_scores['mse']
                torch.save(model.module.state_dict(),
                           "./weights/best_model({}_{}-epoch).pth".format(args.data_path.split('/')[-1], epoch))

                src_file_list = glob(attn_dir + '*.npz')  # glob获得路径下所有文件，可根据需要修改
                for srcfile in src_file_list:
                    move_file(srcfile, best_attn)

                src_file_list = glob(pred_true_value_dir + '*.npz')  # glob获得路径下所有文件，可根据需要修改
                for srcfile in src_file_list:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prt_threshold', type=float, default=0.75,
                        help='Maximum length threshold of protein sequence (default: 0.8)')
    parser.add_argument('--cpd_threshold', type=float, default=0.9995,
                        help='Maximum number of atoms threshold of compound (default: 0.9995)')
    # 是否启用SyncBatchNorm
    parser.add_argument('--syncBN', type=bool, default=True)

    # 数据集所在根目录
    parser.add_argument('--data-path', type=str, default="./data/kiba")
